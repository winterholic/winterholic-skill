...
CSS_FRAGMENT_RE = re.compile(r"url\(\s*['\"]?#([A-Za-z_][\w:.-]*)['\"]?\s*\)", re.IGNORECASE)
CSS_REFERENCE_ATTRS = {
    "style",
    "fill",
    "stroke",
...
class DiagramParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        for key, value in normalized_attrs:
            ...
            self.fragment_references.extend(CSS_FRAGMENT_RE.findall(value))
...
def check_fragment_integrity(parser: DiagramParser, errors: list[str]) -> None:
    ...
    references = set(parser.fragment_references)
    references.update(CSS_FRAGMENT_RE.findall("\n".join(parser.styles)))
    for identifier in sorted(references - set(parser.ids)):
        errors.append(f"unresolved fragment reference #{identifier}")
...
def verify(path: Path) -> list[str]:
    ...
    check_fragment_integrity(parser, errors)
