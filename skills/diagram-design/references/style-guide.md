...
Default skin is the restrained `winterholic-base` palette: Bright Snow paper, cool-neutral ink, AA-safe brand-blue focal accents, Frozen Lake links, and Twilight depth. The upstream editorial geometry and typography stay intact; only the visual skin changes. Swap these values (or run [`onboarding.md`](onboarding.md)) and every new diagram inherits the new skin without touching any type-specific logic.
...
Every token is referred to by **semantic role**, not by its hex value. Type references (`type-*.md`) and SKILL.md say `accent`, not `#2081C3`.

| Role | Purpose | Default (light) | Default (dark) |
|---|---|---|---|
| `paper` | Page background, default node fill | `#F7F9F9` (Bright Snow) | `#1B2027` |
| `paper-2` | Diagram container bg, secondary fill | `#FFFFFF` | `#2B3139` |
| `ink` | Primary text, primary stroke | `#2B3139` | `#F2F7FF` |
| `muted` | Secondary text, default arrow stroke | `#535A63` | `#BCC3CE` |
| `soft` | Sublabels, boundary labels | `#64748B` | `#9FA7B1` |
| `rule` | Hairline borders | `rgba(43,49,57,0.12)` | `rgba(242,247,255,0.12)` |
| `rule-solid` | Stronger borders, baselines | `#D4DBE6` | `#3E444D` |
| `accent` | Focal / 1–2 max per diagram | `#1A76B4` (action primary) | `#69ADE5` |
| `accent-tint` | Fill for accent-bordered boxes | `rgba(26,118,180,0.10)` | `rgba(105,173,229,0.12)` |
| `link` | HTTP/API calls, external arrows | `#0A5E93` | `#63D2FF` (Frozen Lake) |

> **Token source:** `winterholic-design-system/winterholic-base` v1.1.6, upstream commit `cb21b49c02378bc792016dfa83ed0ac6a46728c8`. Semantic roles map from `color.surface.*`, `color.text.*`, `color.border.*`, `color.action.primary.*`, and `color.chart.*`; raw ramp values are not invented locally.

> **Baked assets:** templates, gallery chrome, and pre-baked examples are recolored to this mapping so the examples match the default style guide. Terminal variants intentionally keep their fixed terminal palette.

### Inversion rule (light → dark)

Any `rgba(43,49,57, X)` in light becomes `rgba(242,247,255, X)` in dark. Preserve opacity; switch the RGB base with the semantic `ink` token. The accent uses the dark action-primary token; Frozen Lake remains the distinct link color.
...
|---|---|---|---|
| `series-1` | `#67B6B7` (Pearl Aqua) | `#78D5D7` | Non-focal series |
| `series-2` | `#43568D` (Indigo) | `#98A6C8` | Non-focal series |
| `series-3` | `#EBBB45` (Amber) | `#FAC748` | Non-focal series |
| `series-4` | `#9B7EDE` (Periwinkle) | `#AE94ED` | Non-focal series |
| `series-5` | `#F88DAD` (Bubblegum) | `#FFA3BD` | Non-focal series |
...
| `terminal-ink` | `#f5f5f5` | Primary text and stroke in the fixed terminal skin |
