...
**From Markdown tables:** look for rows with hex values (`#rrggbb`) adjacent to role-like words. A row like `| accent | #1A76B4 |` maps directly.
...
```json
{ "colors": { "brand": { "primary": { "value": "#1A76B4", "type": "color" } } } }
```
