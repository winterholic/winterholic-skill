...
- **Y-axis gridlines:** 4–6 horizontal lines at regular intervals. Stroke `rgba(43,49,57,0.08)` (very faint), 0.8px. X-axis baseline at `rgba(43,49,57,0.25)`, 1px.
...
<rect x="X" y="Y" width="W" height="H" fill="#F7F9F9"/>
<!-- Bar body -->
<rect x="X" y="Y" width="W" height="H" fill="rgba(83,90,99,0.15)" stroke="#535A63" stroke-width="1"/>
<!-- Value label above bar -->
<text x="X+W/2" y="Y-8" fill="#535A63" font-size="8" font-family="'Geist Mono', monospace" text-anchor="middle">VALUE</text>
```

Focal bar: replace fill with `rgba(26,118,180,0.12)`, stroke with `#1A76B4`, label fill with `#1A76B4`.
...
- **Gridlines:** vertical at each tick, `rgba(43,49,57,0.08)` 0.8px, spanning y 56→408. At the domain floor the axis line replaces the gridline rather than doubling it: `rgba(43,49,57,0.25)` 1px, y 40→420.
...
- **Connector:** `rgba(43,49,57,0.55)` 1px, declared before both dots so the dots cap it. It is not the axis hairline: the connector is what says *these two dots are one row*, so it has to clear 3:1 (0.55 gives 3.19:1; the 0.25 the axis uses gives 1.60:1).
...
<line x1="458" y1="96" x2="740" y2="96" stroke="rgba(43,49,57,0.55)" stroke-width="1"/>
<circle cx="458" cy="96" r="6" fill="#F7F9F9" stroke="#535A63" stroke-width="1.5"/>
<circle cx="740" cy="96" r="6" fill="#1A76B4" stroke="#2B3139" stroke-width="1"/>
<text x="446" y="100" fill="#535A63" font-size="8" font-family="'Geist Mono', monospace" text-anchor="end">34</text>
<text x="752" y="100" fill="#535A63" font-size="8" font-family="'Geist Mono', monospace">71</text>
<text x="188" y="100" fill="#2B3139" font-size="11" font-weight="600" font-family="'Geist', sans-serif" text-anchor="end">Platform</text>
```
...
**Dark theme.** Dots and labels swap as expected — hollow fill `#1B2027` with `#BCC3CE` stroke, solid `#69ADE5` with an `#F2F7FF` stroke, labels `#BCC3CE`. **The hairlines must invert too:** gridlines `rgba(242,247,255,0.08)`, axis `rgba(242,247,255,0.20)`, connector `rgba(242,247,255,0.40)` — the connector again heavier than the axis, for the same 3:1 reason. `rgba(27,32,39,…)` *is* the dark paper colour at every alpha value, so a connector, gridline, or axis carried over from light composites to exactly 1.000:1 — the gap encoding and the scale both vanish.
