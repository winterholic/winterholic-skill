...
- **Carry rules (the bridge grammar).** A horizontal connector line carries the running total across each gap between adjacent bars, drawn at the shared level: from the right edge of one bar to the left edge of the next, `rgba(43,49,57,0.55)` at 1px on light paper (`rgba(242,247,255,0.40)` dark) — the dumbbell's connector weight, because the carry is load-bearing and must clear 3:1, not the 0.25 axis hairline. Each carry declares the running total it transports in `data-carry`.
...
- **Sign is encoded by fill weight, not hue.** Increases take the bar family's default tint (`rgba(83,90,99,0.15)` fill, `muted` stroke). Decreases are hollow — paper fill, same `muted` stroke — so the two directions survive greyscale and colour-vision deficiency without leaning on the accent. The geometry (which end of the bar meets the carry) and the signed printed value are the second and third encodings of the same fact.
- **Totals are the heaviest marks:** `rgba(43,49,57,0.08)` fill with a 1px `ink` stroke. The reader's eye should land on the two anchors first, then walk the bridges.
- **Focal bridge (≤1, optional):** the one contribution the chart exists to show gets the bar family's focal treatment — `rgba(26,118,180,0.12)` fill, `accent` stroke, accent value label. It replaces that bar's sign fill; the signed label and geometry still carry the direction.
...
- **Gridlines / axes:** identical to the bar chart — 4–6 faint horizontal gridlines `rgba(43,49,57,0.08)` 0.8px, y-axis line and x-axis baseline `rgba(43,49,57,0.25)` 1px, tick labels Geist Mono 8px muted right-aligned at x=72, rotated y-axis title Geist Mono 7px tracked.
...
<rect x="112" y="192" width="96" height="228" fill="#F7F9F9"/>
<rect x="112" y="192" width="96" height="228" fill="rgba(43,49,57,0.08)" stroke="#2B3139" stroke-width="1"
      data-role="total" data-value="240" data-name="FY25 base"/>
<text x="160" y="184" fill="#535A63" font-size="8" font-family="'Geist Mono', monospace" text-anchor="middle">240</text>

<!-- Carry: the running total (240) crosses the gap at its own level -->
<line x1="208" y1="192" x2="256" y2="192" stroke="rgba(43,49,57,0.55)" stroke-width="1" data-carry="240"/>

<!-- Increase bridge: +64 rises from 240 to 304 -->
<rect x="256" y="131" width="96" height="61" fill="#F7F9F9"/>
<rect x="256" y="131" width="96" height="61" fill="rgba(83,90,99,0.15)" stroke="#535A63" stroke-width="1"
      data-role="delta" data-value="+64" data-name="Headcount"/>
<text x="304" y="123" fill="#535A63" font-size="8" font-family="'Geist Mono', monospace" text-anchor="middle">+64</text>
```
