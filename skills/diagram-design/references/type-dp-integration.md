...
focal_accent: "#1A76B4"             # one color for all focal components (default = SKILL accent)
dark: false
```
...
Bars span the full zone width minus 16-px padding on each side. Bars marked `focal: true` use `bar_h_focal=56` and accent styling (fill `rgba(focal_accent, 0.08)`, stroke `focal_accent`). Non-focal bars use `bar_h_default=44` with muted styling (fill `rgba(43,49,57,0.05)`, stroke `rgba(43,49,57,0.32)`).
...
All side-column nodes use fixed `w=160 h=64`. Same fill / stroke pattern: fill `rgba(83,90,99,0.06)`, stroke `#64748B`, stroke-width 1.
...
|---|---|---|---|---|---|
| `primary` | `#1A76B4` (focal_accent) | 1.4 | — | `arrow-accent` | Every edge whose endpoint is a `focal: true` component. Also every Trino → consumer edge (serve-flow rule). |
| `secondary` | `#535A63` (muted) | 1.2 | — | `arrow` | Default for internal platform-component edges and source → platform edges that don't touch focal. |
| `federated` | `#0A5E93` (link-blue) | 1.0 | `4,3` | `arrow-link` | Federation queries (e.g., source DB → Trino). |
| `trigger` | `#535A63` (muted) | 1.0 | `4,3` | `arrow` | Every edge originating from a `kind: bar` component (Airflow drops). Unlabelled. |
| `auth` | `#1A76B4` | 1.2 | `5,4` | `arrow-accent` | Every edge from a footer node up to the zone bottom edge. **Never to a specific component.** |
...
<defs>
  ...
  <marker id="arrow-link"   markerWidth="8" markerHeight="6" refX="7" refY="3"   orient="auto"><polygon points="0 0, 8 3, 0 6" fill="#0A5E93"/></marker>
...
| Token | Light | Dark |
|---|---|---|
| Page paper | `#F7F9F9` | `#1B2027` |
| Ink | `#2B3139` | `#F2F7FF` |
| Muted | `#535A63` | `#BCC3CE` |
| Accent | `#1A76B4` | `#69ADE5` |
| Link (federated) | `#0A5E93` | `#63D2FF` |
| Side-column fill | `rgba(83,90,99,0.06)` | `rgba(242,247,255,0.06)` |
| Side-column stroke | `#64748B` | `rgba(242,247,255,0.30)` |
| Zone fill | `rgba(43,49,57,0.025)` | `rgba(242,247,255,0.04)` |
| Zone stroke | `rgba(43,49,57,0.32)` | `rgba(242,247,255,0.30)` |
| Non-focal bar fill | `rgba(43,49,57,0.05)` | `rgba(242,247,255,0.06)` |
