...
| 2 | "FastAPI 응답이 422만 떠" | dev-fastapi 직행 | — | 첫 진단에서 라우터 경유 자체가 감점(스택 특정) |
...
|---|---|---|
| "FastAPI 422가 여전히 나" | systematic-debugging으로 재현·호출경로·환경 고정 → dev-fastapi | 스택이 특정됐다는 이유로 같은 수정 반복 |
| "API가 너무 많아서 느린 듯" | 네트워크·프로파일을 실측해 가설 반증 → 병목 전문가 | 사용자 원인 추정을 사실로 취급 |
| "로컬은 되는데 배포가 안 돼" | 실행 대상·번들·설정·health와 수정 동작을 라이브에서 확인 | build/typecheck 통과로 완료 주장 |
| "UI가 이상해, 화면 봐줘" | ui-refine-loop가 화면 관찰 → 구현 필요 시 프론트 전문가 → 재캡처 | dev-react가 코드부터 수정 |

새 사례는 `사용자 원문 / 당시 증거 / 부르면 안 되는 전문가 / 불변식 / 기대 행동 / 과거 실패`를 함께 보존한다. 라우팅 표 수정 뒤 이전 구현 설명을 숨긴 fresh 세션에서 실제 행동을 확인한다.
