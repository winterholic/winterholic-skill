...
## 공통 실행 계약 — 모든 전문가가 지켜야 할 수명주기

개별 전문가는 지식 카드가 아니라 아래 수명주기의 한 렌즈다. 직행 호출도 이 계약을 생략하지 않는다.

1. **관찰** — 저장소·실행 결과·로그·현재 설정·렌더 화면에서 사실을 먼저 모은다. 사용자와 전문가의 원인 추정은 증거가 아니라 가설이다.
2. **Route capsule** — `목표·증상 / 직접 관측 / 가설·반증 / 범위·비범위 / 불변식 / 성공 기준·검증 표면`을 고정한다.
3. **적합성 응답** — 후보 전문가의 `언제 발동 / 경계`를 대조해 `ACCEPT: 내가 책임질 결정` 또는 `REROUTE: 어긋난 경계 + 다음 후보`를 내부적으로 판정한다. 이름이 비슷하다는 이유로 끝까지 밀지 않는다.
4. **한 단계 확대** — 국소 증상이라도 원인이 component/system/runtime/deploy 경계에 있는지 한 번 본다. 같은 증상이 반복되거나 초기 가설이 기각되면 systematic-debugging을 선행한다.
5. **실행** — 프로젝트 규칙을 먼저 적용하고, 선택한 전문가의 안티패턴·워크플로우 중 현재 결정에 필요한 것만 쓴다. 여러 전문가는 한 작업 소유자가 순차 통합하며 같은 파일을 병렬 수정하지 않는다.
6. **표면별 검증** — 아래 사다리에서 실제로 닿은 층까지 확인한다. 아래층 통과를 위층 통과로 확대 해석하지 않는다.
7. **Execution receipt** — 종료 시 `적용한 전문가·핵심 결정 / 관측 증거 / 실행한 검증과 결과 / 미검증 표면 / 남은 가설·재라우팅·학습 후보`를 짧게 남긴다.

증거 우선순위는 `현재 저장소·실행·로그·실환경 > 설치 버전에 맞는 공식 문서 > 전문가 매뉴얼 > 일반 지식`이다. 충돌하면 왼쪽이 이긴다.

### 개발 검증 사다리

| 변경 표면 | 최소 증거 | 이것만으로 주장할 수 없는 것 |
|---|---|---|
| 정적 | lint·typecheck·build | 실제 기능 동작 |
| 기능 | 좁은 테스트·재현 명령·브라우저 상호작용 | 외부 의존성·배포 환경 |
| 통합 | 실제 DB/API/큐/권한 경계 | 운영 배포 상태 |
| 운영 | 대상 환경 health·버전·수정 동작·로그 | 장기 성능·복구성(별도 관찰 필요) |

검증할 수 없는 층은 `미실행: <이유>` 또는 `확인 필요: <표면>`으로 남긴다. 빌드 성공만으로 “고쳤다”고 하지 않는다.

## 공통: 빠른 사용 (Quick Start)

1. **라우터부터** — `dev-chief-architect/SKILL.md`를 읽어 작업을 [언어 × 프레임워크 × 방법론 × 품질] 축으로 분해, 주도 전문가 1명을 고른다. (스택이 명확하면 직행 가능)
2. **적합성 확인** — 후보 SKILL의 경계가 route capsule의 핵심 결정과 맞는지 ACCEPT/REROUTE한다.
3. **필요한 규칙만 실행** — 보강 전문가는 실제 게이트가 생길 때 순차 호출한다.
4. **검증 사다리와 receipt로 닫기** — 반복 오류면 systematic-debugging으로 재현·실행 경로·환경을 먼저 고정한다.

팩 배선 검사: `python ~/.claude\scripts\expert_pack_check.py --pack dev`

## 트러블슈팅 일지 (피드백 루프 — stock-scorecard의 이식)

개발의 사후 채점은 **"같은 삽질 두 번 안 하기"**다. 시간은 관측하기 어려우므로 사건으로 판정한다.

- 학습 후보: 동일 버그 2차 시도, 초기 가설 기각, 배포·설정을 코드 버그로 오인, 재사용 가능한 근본원인 확정.
- 일반 작업 중 공용 ledger를 자동 수정하지 않는다. 종료 보고에 `학습 후보: 증상 → 근본 원인 → 재발 방지` 한 줄만 남긴다.
- 하네스 갱신 요청 때 work-history에서 같은 유형이 2~3회 반복됐는지 확인해 해당 전문가의 안티패턴·회귀 eval로 승격한다. 기존 `troubleshooting/ledger.md`는 기록이 있을 때만 보조 자료로 쓴다.
...
| dev-llm-engineering | 프롬프트·RAG·에이전트 (공급자 API 세부는 설치된 전용 스킬 또는 공식 문서) | 3 | ✅ |
...
| 공급자 API 레퍼런스 | 설치된 전용 스킬이 있으면 위임, 없으면 최신 공식 문서 |
| 주식 '분석·판단' | stock-experts 디스패처 |

## 상태

**전 Phase 완료(2026-06-12)**: 전문가 91종 + 라우터 1 = `dev-*` 폴더 92개. 메타 규칙 1개를 포함한 문서상 체계는 93종이다. 호출 시 산문 숫자보다 실제 `SKILL.md` 존재를 확인한다.
