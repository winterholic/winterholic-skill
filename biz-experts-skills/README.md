...
4. **사후 채점(결정의 검증 가능성)**: biz 결정은 결과가 늦게 온다. 모든 산출물에 **가설 · 성공지표 · 언제 확인할지 · 뒤집을 조건**을 명시한다. 기록은 사용자가 요청했을 때 프로젝트 로컬 결정 기록에 남긴다.
...
## 공통 실행 계약 — 프레임워크를 결정으로 바꾸는 수명주기

개별 전문가는 직무 이름표가 아니라 의사결정 렌즈다. 직행 호출도 아래 계약을 지킨다.

1. **관찰** — 현재 산출물·화면·사용자 행동·분석 데이터·프로젝트 규칙을 먼저 본다. 사용자의 느낌은 버리지 않되 원인으로 확정하지 않는다.
2. **Route capsule** — `목표·증상 / 직접 관측 / 가설·반증 / 범위·비범위 / 불변식 / 결정 / 성공지표·확인 시점`을 고정한다.
3. **적합성 응답** — 후보 전문가의 경계를 대조해 `ACCEPT: 내가 책임질 결정` 또는 `REROUTE: 어긋난 경계 + 다음 후보`를 판정한다. 산출물 명사보다 움직여야 할 결정·outcome을 따른다.
4. **한 단계 확대** — 문구·간격·캠페인 같은 국소 요청도 원인이 flow/system/positioning/PMF에 있는지 한 번 본다. 확대는 재설계 허가가 아니라 원인 층 판별이다.
5. **실행** — 프로젝트·브랜드 규칙을 우선하고, 되돌리기 쉬운 최소 실험부터 한다. 여러 전문가는 한 소유자가 순차 통합하며 상충 권고를 그대로 병렬 산출하지 않는다.
6. **검증** — 현재 산출물 검토 → 실제 행동·데이터 → 제한된 실험 → 확인 시점 순으로, 가능한 층까지만 증명한다.
7. **Decision receipt** — `적용한 전문가·결정 / 근거 / 가설 / 성공지표·확인 시점 / 뒤집을 조건 / 미검증·재라우팅`을 남긴다.

증거 우선순위는 `현재 산출물·행동·데이터·프로젝트 규칙 > 현재 1차 자료·플랫폼 정책 > 전문가 프레임 > 일반론`이다.

**이미 운영 중인 화면·정책을 고치는 작업은 신규 설계와 다른 규율을 탄다** — 12종 전문은 `existing-screen-edit.md`(실사용 사고 기록 기반)이고, 요약 훅은 디자인·기획 계열 5종(ui-designer·ux-designer·service-planner·design-system·product-designer) SKILL 본문에 들어 있다. 실행 도구는 `biz-ui-designer/scripts/`(화면 계측·세로 예산) · `biz-design-system/scripts/`(토큰 난립) · `biz-product-designer/scripts/`(결정 영수증)에 있고, 셋 다 `--self-test` 로 자기 검사가 실패할 수 있는지부터 확인한다.

## 공통: 빠른 사용 (Quick Start)

1. **라우터부터** — `biz-chief-strategist/SKILL.md`를 읽어 [기능 × 단계 × 결정 × 지표]로 분해, 주도 전문가 1명을 고른다. (직무가 명확하면 직행 가능)
2. **적합성 확인** — 후보 SKILL의 경계가 route capsule의 핵심 결정과 맞는지 ACCEPT/REROUTE한다.
3. **필요한 규칙만 실행** — 보강 전문가는 선행 결과가 게이트를 만들 때 순차 호출한다.
4. **Decision receipt로 닫기** — 큰 결정은 가설·확인 시점·뒤집을 조건을 남기며, 존재하지 않는 공용 ledger를 가정하지 않는다.

팩 배선 검사: `python ~/.claude\scripts\expert_pack_check.py --pack biz`

## 사후 채점 — biz판 "같은 실수 두 번 안 하기"

코드와 달리 마케팅·전략·제품 결정은 결과가 수주~수개월 뒤에 온다. 그래서:

- 되돌리기 어려운 큰 결정(가격 변경·리브랜딩·핵심 기능 방향·캠페인 예산)은 산출물에 다음 필드를 남긴다. 사용자가 기록을 요청했을 때만 프로젝트 로컬 기록에 append한다:
  `| 날짜 | 영역 | 결정 | 가설 | 성공지표 | 확인 시점 | 뒤집을 조건 | 결과(나중에) |`
