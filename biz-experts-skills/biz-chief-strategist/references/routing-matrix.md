...
> 회귀 픽스처의 SSOT는 `../evals/routing-cases.json`이다. 아래 표는 사람용 해설이며, 실사용 실패는 JSON에 append하고 팩 검사기로 경로·스키마를 검증한다.
...
### 실사용 회귀 케이스 — 라벨보다 행동을 채점한다

정답은 전문가 이름만이 아니다. 다음 사례는 **첫 관찰·불변식·호출 순서·검증 행동**까지 맞아야 통과한다.

| 실제 발화 유형 | 기대 행동 | 실패 행동 |
|---|---|---|
| "패딩이 이상하고 답답해" | ui-refine-loop로 화면·영역을 고정하고 padding·고정크기·grid·scroll 경쟁 가설을 실측 → 구조 문제만 biz-ui/ux로 | padding 값부터 바꿈 |
| "UI가 다 깨져" | 문제 감각은 보존하되 화면·뷰포트·라이브/로컬을 확인하고 범위를 좁힘 | 사용자 원인 설명을 그대로 확정하거나 반대로 무시 |
| "이 화면 좀 완성도 있게" | 기존 자산·크기·기능·데스크톱/모바일 불변식을 route capsule에 고정 → ui-refine-loop 관찰 → 필요 시 biz 렌즈 → dev 구현 → 재캡처 | 새 디자인으로 갈아엎거나 모바일 한 장만 확인 |
| "API가 너무 많아서 느린 듯" | 실제 네트워크·타이밍을 측정해 가설을 반증한 뒤 병목에 맞는 biz/dev 경계 선택 | 발화 속 기술 원인을 사실로 취급 |

새 회귀 케이스는 work-history의 `사용자 원문 + 당시 증거 + 부르면 안 되는 전문가 + 불변식 + 기대 행동 + 과거 실패`를 함께 보존한다. 자가 채점만으로 출고하지 않고, 이전 구현 설명을 숨긴 fresh 세션에서 실제 행동을 확인한다.
